...
myhostname=$(hostname) # Dynamic hostname 
interface_name=$(ip a |awk '/: e/{gsub(/:/,"");print $2}') #Getting interface name
lan_address=$(ip a s $interface_name|awk '/inet /{gsub(/\/.*/,"");print $2}') #getting lan address from ip a s ename
lan_hostname=$(getent hosts $(ip a s $interface_name)|awk '/inet /{gsub(/\/.*/,"");print $2}' | awk '{print $2}') # lan hostname from hosts file
external_ip=$(curl -s icanhazip.com) #external ip getting from website
external_name=$(getent hosts $external_ip| awk '{print $2}') # getting external name from hosts file
router_address=$(ip r s default| cut -d ' ' -f 3) #router address from ip r s
router_hostname=$(getent hosts $router_address|awk '{print $2}') # router hostname from hosts file /etc/hosts
