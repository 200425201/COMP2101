...
array=$(lshw -class network |  awk '/logical name:/{print $3}')
declare -a intlist=($array)
#converting string into array
len=${#intlist[*]}
i=0
#loop to go thrugh everything in array
while [ $i -lt $len ]
do
    if [[ ${intlist[$i]} = lo* ]] ;
	then continue ; fi
	#skipping the loopback a
	 ipv4_address=$(ip a s ${intlist[$i]} | awk -F '[/ ]+' '/inet /{print $3}')
	 ipv4_hostname=$(getent hosts $ipv4_address | awk '{print $2}')
	 network_address=$(ip route list dev ${intlist[$i]} scope link|cut -d ' ' -f 1)
	 network_number=$(cut -d / -f 1 <<<"$network_address")
	 network_name=$(getent networks $network_number|awk '{print $1}')
#Once per host report with printing variables
echo Interface ${intlist[$i]}:
echo ===============
